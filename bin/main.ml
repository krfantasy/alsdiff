...
let () =
  ...
  let result1 = Alsdiff_lib.Upath.find sample_xml "/root/a/b" in
  (match result1 with
  ...
  let result2 = Alsdiff_lib.Upath.find sample_xml "/a/b" in
  (match result2 with
   | Some (p, _) -> Printf.printf "Found path: %s\n" p
   | None -> Printf.printf "Path not found\n");
  ...
  let result3 = Alsdiff_lib.Upath.find sample_xml "/xyz/a/b" in
  match result3 with
  | Some (p, _) -> Printf.printf "Found path: %s\n" p
  | None -> Printf.printf "Path not found\n"
