...
let alsflow_cmd ~config ~domain_mgr : int =
  ...
  let output = Flowchart.render_flowchart ~xml ~liveset ~options in
  Fmt.pr "%s@." output;
  0
...
let cmd =
  ...
  let config = { file; direction; include_external; include_routing; include_sends } in
...
let () =
  let info =
    Cmd.info "alsflow" ~doc:"Render ALS track routing as Mermaid flowchart"
  in
